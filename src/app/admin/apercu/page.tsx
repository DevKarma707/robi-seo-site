...
import { ACCENT, btn, btnGhost, btnPill, btnPrimary, card, focusRing, kpiLabel, kpiValue, sectionTitle } from "@/components/admin/ui";
    ...
    <div className="a-shell min-h-screen flex">
      <aside className="a-sidebar w-[232px] flex-shrink-0 flex flex-col h-screen sticky top-0">
        ...
        <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
          {NAV.map((t) => (
            <button
              key={t.label}
              className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl text-[13px] font-semibold transition-all ${focusRing} ${t.active ? navActive : navItem}`}
            >
              {t.icon}
        ...
        <div className="p-3 border-t border-white/[0.06] space-y-0.5">
          ...
          <span className="w-full flex items-center gap-3 px-3 py-2 rounded-xl text-[13px] font-semibold text-red-400">
      ...
      <main className="flex-1 overflow-auto">
        <div className="mx-auto w-full max-w-[1600px] px-6 py-8 md:px-10">
          <div className="mb-8">
            <h1 className="font-black text-[32px] leading-none tracking-[-0.02em] text-white">Pilotage</h1>
            <p className="text-[13px] mt-2.5 text-white/40">Inscriptions, activation, usage et funnel — agrégats uniquement</p>
          </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {KPIS.map((k) => (
                <div key={k.label} className={`${card} a-card-hover a-kpi p-4`}>
                  ...
                  {k.sub && <p className="text-[11px] mt-2 text-white/35">{k.sub}</p>}
                </div>
              ))}
              <div className={`${card} a-card-hover a-kpi p-4`}>
                <p className={`${kpiLabel} mb-2`}>{KPI_NODE.label}</p>
                <span className={kpiValue}><span className="text-white/30 text-xl">index en cours</span></span>
              </div>
            </div>
              ...
              <div className="space-y-3">
                {FUNNEL.map((s) => (
                  <div key={s.label} className="flex items-center gap-3">
                    <span className="text-[11px] w-52 text-white/60">{s.label}</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-2.5">
                  {COUNTRIES.map((c) => (
                    <div key={c.name} className="flex items-center gap-3">
                      <span className="text-[11px] w-28 truncate text-white/60">{c.name}</span>
                      <div className="flex-1 h-1.5 rounded-full bg-white/10 overflow-hidden">
                        <div className="a-bar h-full rounded-full" style={{ width: `${(c.count / 11) * 100}%` }} />
                      </div>
                      <span className="text-xs font-black w-6 text-right text-white tabular-nums">{c.count}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className={`${card} p-5`}>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button className={`${btnPill} bg-white/10 text-white/60`}>P2</button>
                </div>
                <div className="mt-5 space-y-2">
                  {["Cocher subscription.active dans Polar", "Faire un achat réel à 59 €"].map((t) => (
                    <div key={t} className="rounded-xl border border-white/10 bg-white/[0.04] p-3">
                      <div className="flex items-start gap-2">
                        <span className="mt-[5px] w-1.5 h-1.5 rounded-full flex-shrink-0 bg-[#f87171]" />
                        <p className="text-[12px] font-semibold leading-snug flex-1 text-white">{t}</p>
                      </div>
                      <div className="flex items-center gap-1.5 mt-2 pl-3.5">
                        ...
                        <span className="text-[9px] text-white/35">Ralph</span>
                        <span className="text-[9px] text-white/25">&lt; 30 min</span>
                      </div>
                    </div>
                  ))}
                </div>
