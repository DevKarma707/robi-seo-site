      ...
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center h-14 gap-8">
          ...
          <div className="hidden lg:flex items-center gap-6 flex-1 justify-center">
            {navigation.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="text-sm font-medium text-white/70 hover:text-[#BEF221] transition-colors"
              >
                {item.name}
              </Link>
            ))}
          ...
          <div className="hidden lg:flex items-center gap-4 flex-shrink-0 ml-auto">
            ...
            <Button href="https://app.robi-app.com" size="sm">
              {dict.nav.login}
            </Button>
          </div>
          ...
          <div className="flex lg:hidden items-center gap-2 ml-auto">
            <LanguageSwitcher currentLocale={locale} />
            <Button href="https://app.robi-app.com" size="sm" className="text-xs px-3 py-1.5">
              {dict.nav.login}
            </Button>
            <button
